...
class VerificationWorker:
    ...
    def __init__(
        ...
        Args:
            ...
            openai_api_key: OpenAI API key for refinement (legacy)
            openai_model: Model name for refinement (legacy)
        ...
        if model_backend_config:
            # New unified backend approach
            try:
                backend_type = model_backend_config.get("backend_type", "openai")
                backend_config = model_backend_config.get(backend_type, {})
                ...
                self.logger.info(
                    f"Initialized {backend_type} model backend: {self.model_backend.model_name}"
                )
        ...
        if self.model_backend is None and (
            OPENAI_AVAILABLE
            and openai_api_key
    ...
    def _refine_kernel(
        ...
        """
        Refine kernel based on error information using model backend.
        ...
        if self.model_backend and self.model_backend.is_available():
            try:
                self.logger.info(
                    f"Refining kernel using {self.model_backend.model_name}"
                )
                ...
                response_text = self.model_backend.generate_completion(
                    prompt, max_tokens=32000, temperature=0.7
                )
                ...
                if refined_kernel:
                    self.logger.info(
                        f"Successfully refined kernel using {self.model_backend.model_name}"
                    )
                ...
                self.logger.error(f"Error refining kernel with model backend: {e}")
                # Fall back to legacy OpenAI client if available

        # Legacy OpenAI client fallback
        if self.openai_client:
            try:
                self.logger.info("Refining kernel using legacy OpenAI API")
                ...
                if refined_kernel:
                    self.logger.info("Successfully refined kernel using legacy OpenAI")
                    return refined_kernel
                else:
                    self.logger.error(
