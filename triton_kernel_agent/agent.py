...
class TritonKernelAgent:
    ...
    def __init__(
        ...
        model_backend_config: Optional[Dict[str, Any]] = None,
        ...
        Args:
            ...
            model_name: OpenAI model to use (loaded from .env if None, legacy)
            high_reasoning_effort: Whether to use high reasoning effort for OpenAI models (legacy)
            model_backend_config: Configuration for model backend (new unified approach)
            backend_config_path: Path to backend configuration file
        """
        ...
        # Initialize model backend configuration
        if model_backend_config:
            self.model_backend_config = model_backend_config
        elif backend_config_path:
            self.model_backend_config = load_backend_config(backend_config_path)
        else:
            # Load default configuration from environment
            self.model_backend_config = load_backend_config()

        # Initialize model backend for agent operations (test/kernel generation)
        self.model_backend = None
        self.openai_client = None  # Keep for backward compatibility
        
        # Initialize logger first
        self.logger = logging.getLogger("TritonKernelAgent")
        
        try:
            backend_type = self.model_backend_config.get('backend_type', 'openai')
            backend_config = self.model_backend_config.get(backend_type, {})
            
            # Add proxy configuration for OpenAI if needed
            if backend_type == 'openai':
                proxy_config = _get_meta_proxy_config()
                if proxy_config:
                    backend_config['proxy_config'] = proxy_config
            
            self.model_backend = create_model_backend(backend_type, **backend_config)
            self.logger.info(f"Initialized {backend_type} model backend: {self.model_backend.model_name}")
            
        except Exception as e:
            self.logger.error(f"Failed to initialize model backend: {e}")
            self.logger.info("Falling back to legacy OpenAI client")
            self.model_backend = None

        # Legacy OpenAI client initialization (fallback)
        if self.model_backend is None and OPENAI_AVAILABLE:
        ...
        self.manager = WorkerManager(
            ...
            model_backend_config=self.model_backend_config,
        )
    ...
    def _generate_test(
        ...
        """
        Generate test code for the problem using model backend.
        ...
        """
        # Try new model backend first
        if self.model_backend and self.model_backend.is_available():
            try:
                self.logger.info(f"Generating test code using {self.model_backend.model_name}")
                ...
                # Generate completion using model backend
                response_text = self.model_backend.generate_completion(
                    prompt, 
                    max_tokens=32000,
                    temperature=0.7
                )
                ...
                if test_code:
                    self.logger.info(f"Successfully generated test code using {self.model_backend.model_name}")
                    return test_code
                else:
                    self.logger.error(
                        "Failed to extract valid code from model response"
                    )
                    raise ValueError("No valid code found in model response")

            except Exception as e:
                self.logger.error(f"Error generating test with model backend: {e}")
                # Fall back to legacy OpenAI client if available

        # Legacy OpenAI API fallback
        if self.openai_client:
            try:
                self.logger.info(f"Generating test code using legacy {self.model_name}")
                ...
                if test_code:
                    self.logger.info("Successfully generated test code using legacy OpenAI")
                    return test_code
                else:
                    self.logger.error(
...
'''
    ...
    def _generate_kernel_seeds(
        ...
        """
        Generate initial kernel implementations using model backend.
        ...
        if self.model_backend and self.model_backend.is_available():
            try:
                self.logger.info(
                    f"Generating {num_seeds} kernel seeds using {self.model_backend.model_name}"
                )
                ...
                # Generate multiple completions using model backend
                response_texts = self.model_backend.generate_multiple_completions(
                    prompt, 
                    n=num_seeds,
                    max_tokens=32000,
                ...
                if kernels:
                    self.logger.info(
                        f"Successfully generated {len(kernels)} kernel seeds using {self.model_backend.model_name}"
                    )
                    return kernels
                else:
                    self.logger.error(
                        "Failed to extract any valid kernels from model responses"
                    )
                    raise ValueError(
                        "No valid kernel code found in any model response"
                    )

            except Exception as e:
                self.logger.error(f"Error generating kernels with model backend: {e}")
                # Fall back to legacy OpenAI client if available
        ...
        if self.openai_client:
            try:
                self.logger.info(
                    f"Generating {num_seeds} kernel seeds using legacy {self.model_name}"
                )
                ...
                if kernels:
                    self.logger.info(
                        f"Successfully generated {len(kernels)} kernel seeds using legacy OpenAI"
                    )
                    return kernels
                else:
                    self.logger.error(
