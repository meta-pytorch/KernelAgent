...
def format_gpu_specs_for_prompt(specs: Dict[str, Any]) -> str:
    """
    Format GPU specifications as a readable string for LLM prompts.

    Args:
        specs: GPU specifications dictionary from get_gpu_specs()

    Returns:
        Formatted string with GPU specifications

    Example:
        >>> specs = get_gpu_specs("NVIDIA A100")
        >>> print(format_gpu_specs_for_prompt(specs))
        GPU: NVIDIA A100 (Ampere)
        - Peak Memory Bandwidth: 1555 GB/s
        - Peak FP32 Performance: 19.5 TFLOPS
        - Peak FP16 Performance: 312.0 TFLOPS
        - SM Count: 108
        - L1 Cache: 192 KB per SM
        - L2 Cache: 40 MB
    """
    lines = [
        f"GPU: {specs['name']} ({specs.get('architecture', 'Unknown')})",
        f"- Peak Memory Bandwidth: {specs['peak_memory_bw_gbps']} GB/s",
        f"- Peak FP32 Performance: {specs['peak_fp32_tflops']} TFLOPS",
    ]

    if specs.get("peak_fp16_tflops", 0) > 0:
        lines.append(f"- Peak FP16 Performance: {specs['peak_fp16_tflops']} TFLOPS")

    if specs.get("peak_bf16_tflops", 0) > 0:
        lines.append(f"- Peak BF16 Performance: {specs['peak_bf16_tflops']} TFLOPS")

    lines.extend(
        [
            f"- SM Count: {specs['sm_count']}",
            f"- Max Threads per SM: {specs['max_threads_per_sm']}",
            f"- L1 Cache: {specs['l1_cache_kb']} KB per SM",
            f"- L2 Cache: {specs['l2_cache_mb']} MB",
            f"- Memory: {specs['memory_gb']} GB {specs.get('memory_type', '')}",
        ]
    )
...
if __name__ == "__main__":
    ...
    # Get specs
    specs = get_gpu_specs()
    print(f"\n{format_gpu_specs_for_prompt(specs)}")
