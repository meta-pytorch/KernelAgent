...
# ---------------------------------------------------------------------------
# Pre-compiled kernel API (quack-style _compile_rmsnorm_fwd)
# ---------------------------------------------------------------------------

_COMPILE_FWD_CACHE: dict[tuple, object] = {}


def _make_fake_tensor(
    dtype: type, shape: tuple, divisibility: int = 1, leading_dim: int = -1
):
    """Create a symbolic tensor for cute.compile (same as quack's fake_tensor)."""
    if leading_dim < 0:
...
def _compile_rmsnorm_fwd(
    dtype: type,
    out_dtype: type | None,
    bias_dtype: type | None,
    weight_dtype: type | None,
    residual_dtype: type | None,
    residual_out_dtype: type | None,
    N: int,
    store_rstd: bool,
    has_residual: bool,
    has_bias: bool,
) -> object:
    """Pre-compile the RMSNorm forward kernel and return a callable.

    Returns a TVM-FFI compiled callable with signature::

        kernel(x, weight, bias, residual, out, residual_out, rstd, mean, eps)

    where each argument is a torch.Tensor (or None) and eps is a float.
    DLPack tensor conversion happens at C++ level — zero Python overhead
    per call.  This matches the quack ``_compile_rmsnorm_fwd`` API.
    """
    key = (
        dtype, out_dtype, bias_dtype, weight_dtype,
        residual_dtype, residual_out_dtype, N,
        store_rstd, has_residual, has_bias,
    )
    cached = _COMPILE_FWD_CACHE.get(key)
    if cached is not None:
        return cached

    # Resolve schedule once.
    direct_gmem = _direct_gmem_from_policy(
        default=bool(dtype.width == 16 and N in {128, 4096, 6144, 7168, 8192})
    )
    use_async = not direct_gmem
    copy_bits = _copy_bits_from_policy(default=128, can_use_256=False)
    stage = 1
    ...
    # Build symbolic tensors for compilation (same as quack's fake_tensor).
    batch_sym = cute.sym_int()
    all_dtypes = [dtype, out_dtype, residual_dtype, weight_dtype,
                  bias_dtype, residual_out_dtype]
    import math as _math
    div = _math.gcd(N, *(128 // dt.width for dt in all_dtypes if dt is not None))

    x_fake = _make_fake_tensor(dtype, (batch_sym, N), div)
    out_fake = _make_fake_tensor(out_dtype or dtype, (batch_sym, N), div)
    res_fake = _make_fake_tensor(residual_dtype, (batch_sym, N), div) if has_residual else None
    res_out_fake = _make_fake_tensor(residual_out_dtype, (batch_sym, N), div) if residual_out_dtype else None
    w_fake = _make_fake_tensor(weight_dtype, (N,), div) if weight_dtype else None
    b_fake = _make_fake_tensor(bias_dtype, (N,), div) if bias_dtype else None
    rstd_fake = _make_fake_tensor(cutlass.Float32, (batch_sym,)) if store_rstd else None

    # Compile with TVM FFI — the returned callable accepts torch tensors
    # directly via DLPack at C++ level.
    compiled = cute.compile(
        op,
        x_fake,
        w_fake,
    ...
    _COMPILE_FWD_CACHE[key] = compiled
    return compiled
